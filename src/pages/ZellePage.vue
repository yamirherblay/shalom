  ...
  <q-page class="q-pa-md zelle-page">
    <div class="q-mx-auto" style="max-width: 980px">
      ...
      <div class="text-body1 q-mb-md">
        Enviamos remesas de forma rápida y segura. Trabajamos con la tasa de cambio vigente y
        realizamos el cálculo por ti. Completa el simulador y solicita el servicio por WhatsApp.
      </div>
        ...
        <q-card-section class="row items-center q-col-gutter-md">
          ...
          <div class="col-12 col-sm-6">
            ...
            <q-input
              ...
              :min="0"
              :step="0.1"
              aria-label="Porciento de entrega"
            />
...
const negocio_id = import.meta.env.VITE_NEGOCIO_ID || 1;
// Configurables
...
const resultCurrency = computed<'CUP' | 'USD'>(() =>
  selectedCurrency.value === 'CUP' ? 'CUP' : 'USD',
);

function formatAmount(value: number, currency: 'CUP' | 'USD') {
  const formatted = Intl.NumberFormat(currency === 'USD' ? 'en-US' : 'es-ES', {
...
onMounted(async() => {
  try {
    ...
    if (data) {
      exchangeRate.value = data[0].rate_cup;
      deliveryPercent.value = data [0].rate_usd;
